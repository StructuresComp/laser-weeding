...
=========================
Trains the MeristemPredictor from scratch at multiple dataset sizes
(same 60/20/20 split ratio) and reports test loss + pixel error for each.

Outputs a formatted table at the end and saves results to generalization_results.csv.

Usage:
    python generalization_sweep.py
"""
...
LR = 1e-4
MAX_EPOCHS = 300
PATIENCE = 50  # increased so runs fully converge before stopping

# Fraction of total dataset held out as a FIXED test set (used for every sweep run).
# Brian's feedback: variable small test sets cause high variance — use one big fixed test set.
FIXED_TEST_FRACTION = 0.2

# Dataset sizes = number of labeled images used for train + val combined.
# These come from the remaining pool after the fixed test set is removed.
...
class PigweedTargetingDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Return ground-truth coordinate for pixel error calculation
...
def compute_pixel_errors(model, loader, device):
    """Returns mean and median Euclidean pixel error (in 224x224 space)."""
    model.eval()
    errors = []
    with torch.no_grad():
        for imgs, targets, gt_coords in loader:
            preds = model(imgs.to(device)).squeeze(1).cpu().numpy()  # (B, 224, 224)
            gt = gt_coords.numpy()  # (B, 2)
...
def train_single_run(train_names, val_names, test_names, device, run_label):
    """Train from scratch, return (best_val_loss, test_loss, mean_px_err, median_px_err, epochs_trained)."""
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        # Progress print every 25 epochs
        if (epoch + 1) % 25 == 0:
...
if __name__ == "__main__":
    ...
    # Load all available labeled images
    ...
    # --- Carve off FIXED test set once, used for every sweep row ---
    ...
    print(f"Fixed test set: {len(fixed_test)} images (held out for all runs)")
    print(f"Train+val pool: {pool_size} images\n")

    # Sweep sizes = size of train+val budget drawn from the pool.
    # Include the full pool as the largest point.
    ...
    for n in sizes:
        # 75/25 train/val split inside the budget of n images
        n_train = int(n * 0.75)
        n_val = n - n_train
        print(f"{'='*60}")
        print(f"SWEEP: N={n} (Train={n_train} | Val={n_val} | Test={len(fixed_test)} fixed)")
        print(f"{'='*60}")

        # Subsample deterministically from the train+val pool
        if n < pool_size:
        ...
        test_n = fixed_test  # always the same held-out set
    ...
    # --- SAVE CSV ---
    csv_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "generalization_results.csv")
