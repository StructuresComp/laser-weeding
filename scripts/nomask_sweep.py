"""
Data Generalization Sweep — Heatmap WITHOUT mask multiplication
================================================================
Brian's hypothesis: the original heatmap is multiplied by the foreground mask,
which gives every training sample a unique target shape that the model has to
match pixel-perfectly. That's a wasted learning burden.

This variant keeps the ground-truth heatmap as a clean Gaussian regardless of
where it falls. The model only has to learn LOCATION, not shape.

Same fixed test set, same seed, same model as generalization_sweep.py.
The ONLY change is removing one line in __getitem__ (the mask multiplication).

Usage:
    python nomask_sweep.py
"""
...
# --- CONFIG (identical to generalization_sweep.py) ---
...
# --- DATASET (clean Gaussian — NO mask multiplication) ---
class PigweedTargetingDatasetNoMask(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # KEY DIFFERENCE: no mask multiplication. The Gaussian stays pristine.
        # (Original line was: heatmap *= (gray > 10).astype(np.float32))
...
# --- MODEL (same as generalization_sweep.py) ---
...
if __name__ == "__main__":
    ...
    for n in sizes:
        ...
        n_val = n - n_train
        print(f"{'='*60}")
        print(f"SWEEP (HEATMAP NO-MASK): N={n} (Train={n_train} | Val={n_val} | Test={len(fixed_test)} fixed)")
        print(f"{'='*60}")
    ...
    print("\n" + "=" * 100)
    print("GENERALIZATION SWEEP RESULTS — HEATMAP (NO MASK)")
    print("=" * 100)
    ...
    csv_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "nomask_results.csv")
