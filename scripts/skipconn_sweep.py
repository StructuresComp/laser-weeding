"""
Data Generalization Sweep — Heatmap with Skip Connections (U-Net style)
=======================================================================
Same as generalization_sweep.py but with skip connections from MobileNetV3's
intermediate layers into the decoder. Tests Brian's hypothesis that spatial
detail lost in the bottleneck is why the plain heatmap model struggles on
small datasets.

Architecture:
    Encoder (MobileNetV3-Small):
        Layer 3  → 24ch  @ 28x28  (skip to decoder stage 2)
        Layer 8  → 48ch  @ 14x14  (skip to decoder stage 1)
        Layer 12 → 576ch @  7x7   (bottleneck → decoder input)

    Decoder:
        7x7   → 14x14  (concat with 48ch skip)  → conv
        14x14 → 28x28  (concat with 24ch skip)  → conv
        28x28 → 224x224 (upsample + final conv)  → heatmap

Same fixed test set, same seed as other sweeps for direct comparison.

Usage:
    python skipconn_sweep.py
"""
...
# --- DATASET (same as generalization_sweep.py) ---
...
# --- MODEL: Heatmap with Skip Connections ---
class MeristemPredictorSkip(nn.Module):
    """
    MobileNetV3-Small encoder with U-Net style skip connections.

    Skip taps:
        encoder layers 0-3  → 24ch  @ 28x28
        encoder layers 4-8  → 48ch  @ 14x14
        encoder layers 9-12 → 576ch @  7x7  (bottleneck)
    """
    def __init__(self):
        super().__init__()
        backbone = models.mobilenet_v3_small(weights='DEFAULT').features

        # Split encoder into 3 stages at the resolution boundaries
        self.enc_stage1 = backbone[:4]   # → 24ch @ 28x28
        self.enc_stage2 = backbone[4:9]  # → 48ch @ 14x14
        self.enc_stage3 = backbone[9:]   # → 576ch @ 7x7

        # Decoder stage 1: 7x7 → 14x14, then concat 48ch skip
        self.up1 = nn.ConvTranspose2d(576, 256, 4, 2, 1)  # 7→14
        self.dec1 = nn.Sequential(
            nn.Conv2d(256 + 48, 256, 3, padding=1),  # concat skip
            nn.BatchNorm2d(256),
            nn.ReLU(),
        )

        # Decoder stage 2: 14x14 → 28x28, then concat 24ch skip
        self.up2 = nn.ConvTranspose2d(256, 128, 4, 2, 1)  # 14→28
        self.dec2 = nn.Sequential(
            nn.Conv2d(128 + 24, 128, 3, padding=1),  # concat skip
            nn.BatchNorm2d(128),
            nn.ReLU(),
        )

        # Decoder stage 3: 28x28 → 56x56
        self.up3 = nn.ConvTranspose2d(128, 64, 4, 2, 1)  # 28→56
    ...
    def forward(self, x):
        ...
        s1 = self.enc_stage1(x)   # 24ch @ 28x28
        s2 = self.enc_stage2(s1)  # 48ch @ 14x14
        bot = self.enc_stage3(s2) # 576ch @ 7x7

        # Decoder with skip connections
        d1 = self.up1(bot)                          # 256ch @ 14x14
        d1 = self.dec1(torch.cat([d1, s2], dim=1))  # concat 48ch skip → 256ch @ 14x14

        d2 = self.up2(d1)                           # 128ch @ 28x28
        d2 = self.dec2(torch.cat([d2, s1], dim=1))  # concat 24ch skip → 128ch @ 28x28

        d3 = self.up3(d2)                           # 64ch @ 56x56
        d3 = self.dec3(d3)

        return self.final(d3)                        # 1ch @ 224x224
...
def train_single_run(train_names, val_names, test_names, device, run_label):
    ...
    test_loader = DataLoader(PigweedTargetingDataset(test_names, augment=False),
                             batch_size=BATCH_SIZE, num_workers=0)
...
if __name__ == "__main__":
    ...
    # SAME split as other sweeps (random_state=42)
    ...
    for n in sizes:
        ...
        n_val = n - n_train
        print(f"{'='*60}")
        print(f"SWEEP (SKIP-CONN HEATMAP): N={n} (Train={n_train} | Val={n_val} | Test={len(fixed_test)} fixed)")
        print(f"{'='*60}")
        ...
        print(f"\n  Result: Test MSE={test_loss:.7f} | Mean Px Err={mean_px:.1f} | "
              f"Median Px Err={median_px:.1f} | Epochs={epochs} | Time={elapsed/60:.1f}min\n")

    print("\n" + "=" * 100)
    print("GENERALIZATION SWEEP RESULTS — HEATMAP WITH SKIP CONNECTIONS")
    print("=" * 100)
    ...
    csv_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "skipconn_results.csv")
